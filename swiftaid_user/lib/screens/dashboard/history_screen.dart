...
class _EmergencyHistoryScreenState extends State<EmergencyHistoryScreen> {
  late Future<Map<String, List<Map<String, dynamic>>>> _groupedHistory;
  ...
  Future<Map<String, List<Map<String, dynamic>>>> _fetchAndGroup() async {
    final prefs = await SharedPreferences.getInstance();
    final userId = prefs.getString('userId');

    final res = await http.get(
      Uri.parse(
          'https://swift-aid-backend.onrender.com/user/$userId/emergencies'),
    );
    ...
    // Sort so the newest groups come first
    final ordered = Map.fromEntries(
      grouped.entries.toList()
        ..sort((a, b) {
          DateTime latest(List<Map<String, dynamic>> list) =>
              list.map((e) => DateTime.parse(e['createdAt']))
